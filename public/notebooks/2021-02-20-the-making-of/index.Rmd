...
This post is a working demonstration of the above. In [this follow up post](/notebooks/2021-02-20-testing-rmarkdown-with-gnip-data/), I've manipulated a small data set and generated two plots. Now, I can use notebooks to share work in progress, although I can't limit access or allow interaction with the data, yet.

### Step Two: Shiny

My expectation that I'd be able to run Shiny apps from this site was incorrect, so the next stage involves setting up shinyapps.io for app deployment, and, once that's working, showing those apps in notebooks via iframe. Since the apps won't be part of the Hugo site structure, I can move them into other folders, and have the option to use shinyapps.io directly for sharing interactive data. 

- Create a very small app and test locally
- Set up shinyapps.io account and deploy 
- Create a post with an embedded app 

I have a [post containing an iframe running an app](/notebooks/shiny-apps-with-gnip-data/) and will continue developing the app itself into something more complex.
