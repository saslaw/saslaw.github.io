...
This is a post about creating a website using Hugo, GitHub, and blogdown for Rmarkdown. There are a number of approaches to this setup, where it starts and which tools to use specifically. My approach is based on prior experience with static site generators, markdown, GitHub, and RStudio.
...
Once I had the essential CV live, I thought about including a section of the site for notebooks like this one, created in Rmarkdown to show off new figures. Keeping the rest of the content on Wordpress and adding HTML output from Rmarkdown would have required several intermediate steps and I wasn't sure how they'd work. I've seen other R developers ([Alison Hill](https://alison.rbind.io/post/new-year-new-blogdown/), [Amber Thomas](https://amber.rbind.io/2016/12/19/website/)) use Hugo and GitHub Pages as a publishing solution for Rmarkdown, and I looked into it again, the moment I had my Wordpress page in publishable, if not launchable, condition.
...
If I wasn't interested in using Rmarkdown to publish notebooks to my page, I'd stop here. I could continue to use Hugo in the command line, add new content by writing pages in markdown using Atom or any other editor, and publish to GitHub pages by pushing builds, which is a pretty smooth workflow, and much less pain than Wordpress for a cleaner, prettier result.
...
The method described in the [blogdown documentation](https://bookdown.org/yihui/blogdown/get-started.html) starts in RStudio, and effectively replaces all the tools I used in my Phase One. Starting here might have saved time, but breaking up the process helped me narrow the cause of errors, and understand the order of operations.
...
-   Test serving local Hugo site in RStudio viewer (This was exciting --- instead of using a browser to view the locally hosted site, it's available in the RStudio viewer window, served through Hugo. I think this is impressive.)
-   Use RStudio Addins to create post
-   Here is the post
-   Return to Hugo layout files to create Notebooks section of site and
-   Test publishing from Rmarkdown builds

This post is a working demonstration of the above. In [this follow up post](/notebooks/2021-02-20-testing-rmarkdown-with-gnip-data/), some R code runs and generates a plot. This enables publishing work in progress, and non-secure sharing of code and products with collaborators who don't need to interact with either. 

### Step Two: Shiny

I haven't used Shiny before. Because the purpose of Shiny, to me, is to create interactive pages, I won't separate that development work from the Hugo setup. I'll add Shiny elements to the existing GNIP post, or create a new notebook, and update this notebook accordingly.
