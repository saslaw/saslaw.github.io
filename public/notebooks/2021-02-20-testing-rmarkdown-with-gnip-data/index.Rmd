...
This plot shows time series data. The code is hidden, it is very similar to the above, with the addition of a copy of the data set for time series analysis. 
...
plot.time.temp <- ggplot() +
  geom_point(data=waters.time %>%
             ...
             aes(x=Date.Format, y=Air.Temperature, color=Country)) +
...
```
 ...
 Next, I'd like to plot different variables over time. This could be accomplished with a function that builds ggplots from input variables, but here is where I'll try something new with Shiny. Rendering this .Rmd notebook and building with the two ggplots above is already affecting Hugo performance, so I imagine Shiny will as well.

## Interactive plot (coming soon)

The next step is to add an interactive element using the same GNIP data I have already cleaned. I may test out basic Shiny concepts elsewhere before applying them to this data set.
